    ...
    <Dialog>
          ...
          <CardContent className="p-3 flex justify-between items-center text-sm text-muted-foreground">
            ...
            <div className="flex gap-1">
              {recipe.allergens.map(a => (
                <span key={a} className="w-5 h-5 rounded-full bg-destructive/10 text-destructive text-[10px] flex items-center justify-center font-bold border border-destructive/20" title={a}>
                  {a[0]}
                </span>
              ))}
            </div>
      ...
      <DialogContent className="max-w-md h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          {/* Ingredients */}
          <div>
            <h3 className="text-lg font-heading font-semibold mb-3 border-b pb-1">Ingredients</h3>
                ...
                return (
                  <li key={idx} className="flex justify-between items-center py-1">
                    ...
                    <div className="flex items-center gap-2">
                       {ing.allergens?.map(a => (
                          <span key={a} className="text-[10px] text-destructive font-bold px-1 border border-destructive/30 rounded">
                            {a.toUpperCase()}
                          </span>
                       ))}
          ...
          {/* Steps */}
          <div>
            <h3 className="text-lg font-heading font-semibold mb-3 border-b pb-1">Preparation</h3>
